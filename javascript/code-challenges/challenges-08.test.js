...
const sortByChildren = (charArray) => charArray.sort((a, b) => a.children.length - b.children.length !== 0 ? a.children.length - b.children.length : b.house - a.house);
...
const containsW = (str) => {
  let testString = new RegExp(/w/);
  return testString.test(str);
};
...
const containsWorld = (input) => {
  let testString = new RegExp(/world/);
...
const isCapitalized = (str) => {
  let testString = new RegExp(/\b[A-Z][a-z]+\b/g);
  return str.match(testString) || [];
};
...
const citiesAtoJ = (arr) => {
  return arr.filter((str) => {
    let testString = new RegExp(/[A-J]/);
    return testString.test(str.split(' ')[0]);
  });
};
